...
#  errores=errores[order(hms(errores[,2])),]
  ...
  colnames(resumen)=c("Fecha","Periodo","Requested","Responsed","Msecs","Timeout",unique(gsub("with value.*","",unique(errores[,7]))))
  ...
  if(!is.null(colnames(resumen.1))){
   ...
   if(length(diferencia)!=0){
     ...
     resumen=resumen[,match(colnames(resumen),colnames(resumen.1))]
   }
   diferencia=setdiff(colnames(resumen),colnames(resumen.1))
   if(length(diferencia)!=0){
     nombres=colnames(resumen.1)
     resumen.1=cbind(resumen.1,matrix(NA,ncol=length(diferencia),nrow=nrow(resumen.1),byrow=T))
     colnames(resumen.1)=c(nombres,diferencia)
     resumen.1=resumen.1[,match(colnames(resumen.1),colnames(resumen))]
   }
  }
