gower=function(k,train){
  ...
  knn=mclapply(1:nrow(train),function(i){
    cat(i,"\n")
    Sij=rep(times=nrow(train),x=NA)
    for(j in 1:nrow(train)){
        sij=sapply(nominales, function(k){ifelse(train[j,k]==train[i,k],1,0)})
        sij=c(sij,as.numeric(1-abs(train[j,cualitativos]-train[i,cualitativos])/rangos))
        Sij[j]=mean(sij)
    }
