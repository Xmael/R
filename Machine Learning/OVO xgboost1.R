...
OVO.DINAMICO.1<-function(train,test,model.knn){
...
#           temp=apply(temp,1,which.max)
  ...
  transforma.dinamica<-function(i,pred.matrix,model.knn,clases){
    ...
    numeros.clases=match(names(table(model.knn$CL[i,])),clases) #Toma las clases vecinas que genera model.knn para eliminar filas de clases que no sean vecinas
  ...
  pred.matrix<-OVO(train,test,clases) #Genera la "hipermatriz" de votación
  model.knn=kknn(as.formula(paste(colnames(train)[ncol(train)],"~.")),train=train,test=test,k=3*length(clases))
